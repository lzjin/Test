    ...
    public PlayerManager(final Activity activity, IjkVideoView ijkVideoView) {
        ...
        videoView.setOnCompletionListener(new IMediaPlayer.OnCompletionListener() {
            ...
            public void onCompletion(IMediaPlayer mp) {
                statusChange(STATUS_COMPLETED,mp,0,0);
        ...
        videoView.setOnErrorListener(new IMediaPlayer.OnErrorListener() {
            ...
            public boolean onError(IMediaPlayer mp, int what, int extra) {
                statusChange(STATUS_ERROR,mp,what,extra);
        ...
        videoView.setOnPreparedListener(new IMediaPlayer.OnPreparedListener() {
            ...
            public void onPrepared(IMediaPlayer iMediaPlayer) {
                playerStateListener.onPrepare(iMediaPlayer);
            }
        });
            ...
            public boolean onInfo(IMediaPlayer mp, int what, int extra) {
                ...
                switch (what) {
                    case IMediaPlayer.MEDIA_INFO_BUFFERING_START:
                        statusChange(STATUS_LOADING,mp,what,extra);
                        break;
                    case IMediaPlayer.MEDIA_INFO_BUFFERING_END:
                        statusChange(STATUS_PLAYING,mp,what,extra);
                        break;
...
//                      Toast.show("download rate:" + extra);
    ...
    private void statusChange(int newStatus, IMediaPlayer mp, int what, int extra) {
        status = newStatus;
        if (!isLive && newStatus == STATUS_COMPLETED) {
            MLog.d("statusChange STATUS_COMPLETED...");
            if (playerStateListener != null) {
                playerStateListener.onComplete(mp);
            }
        } else if (newStatus == STATUS_ERROR) {
            MLog.d("statusChange STATUS_ERROR...");
            if (playerStateListener != null) {
...
//            $.id(R.id.app_video_loading).visible();
            if (playerStateListener != null) {
                playerStateListener.onLoading(mp,what,extra);
            }
            MLog.d("statusChange STATUS_LOADING...");
        } else if (newStatus == STATUS_PLAYING) {
            MLog.d("statusChange STATUS_PLAYING...");
            if (playerStateListener != null) {
                playerStateListener.onPlay(mp,what,extra);
            }
        }
    }
    ...
    private void onBrightnessSlide(float percent) {
        ...
        MLog.d("brightness:" + brightness + ",percent:" + percent);
    ...
    public interface PlayerStateListener {
        ...
        void onLoading(IMediaPlayer mp, int what, int extra);

        void onPlay(IMediaPlayer mp, int what, int extra);
