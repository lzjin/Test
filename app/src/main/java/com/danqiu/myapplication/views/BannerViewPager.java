...
public class BannerViewPager extends RelativeLayout implements View.OnTouchListener,ViewPager.OnPageChangeListener{
    ...
    private void initPoint() {
        ...
        for (int i = 0; i < urlList.size(); i++) {
             ...
             if(i==currentIndex){
                 imageView.setImageResource(R.mipmap.ic_banner_point_press);
             }
             else {
    ...
    private void setImageBackground(int selectItemsIndex) {
        for (int i = 0; i < mImageView.length; i++) {
            if (i == selectItemsIndex) {
                mImageView[i].setImageResource(R.mipmap.ic_banner_point_press);

            } else {
                mImageView[i].setImageResource(R.mipmap.ic_banner_point);
            }
        }
    }
}
